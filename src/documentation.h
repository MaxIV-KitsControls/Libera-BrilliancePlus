...
@mainpage Tango Device Server for Libera Brilliance+
...
A single Tango Device Server for Libera Brilliance+ controls a single RAF card inserted in the
 Libera chassis: up to 4 RAF cards can be inserted in the chassis (raf1, raf2, raf3, raf4).

Some attributes are applied only to the RAF card controlled by the device server, while others
 are global and affect the entire Libera.

You must define in the Tango DB a Libera Tango Device Server for each RAF card installed in the
 Libera chassis: so if two RAF cards are present then you have to define two different instances
...
In the Tango database you have to configure the device's property "LiberaBoard" with the ID of the board controlled by
 the device ("raf1", "raf2", "raf3" or "raf4"), then restart the device from Jive.
